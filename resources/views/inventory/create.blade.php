...
@section('content')
<div class="grid grid-cols-1 gap-6 relative" x-data="createInventory()">
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] sm:p-6 shadow-sm">
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h3 class="text-lg font-bold text-gray-800 dark:text-white/90">Tambah Data Inventory</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Silakan isi formulir di bawah ini untuk menambahkan data PC & Remote baru.</p>
            </div>
        ...
        <form action="{{ route('inventory.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50/50 dark:bg-gray-800/20 p-5 rounded-xl border border-gray-100 dark:border-gray-800 mb-8"> 
                ...
                <div>
                    ...
                    <input type="number" id="id_karyawan" name="id_karyawan" value="{{ old('id_karyawan') }}" placeholder="Contoh: 12345" class="w-full rounded-lg border @error('id_karyawan') border-red-500 focus:ring-red-500 @else border-gray-300 focus:border-brand-500 focus:ring-brand-500 @enderror bg-transparent px-4 py-3 text-sm text-gray-800 focus:outline-none focus:ring-1 dark:border-gray-700 dark:text-white/90 shadow-sm" />
                    @error('id_karyawan')
                ...
                <div>
                    ...
                    <input type="text" id="username_ad" name="username_ad" value="{{ old('username_ad') }}" placeholder="Contoh: user.name" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-3 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-1 dark:border-gray-700 dark:text-white/90 shadow-sm" />
                </div>
                <!-- Nama User -->
                <div>
                    <label for="nama_user" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Nama User</label>
                    <input type="text" id="nama_user" name="nama_user" value="{{ old('nama_user') }}" placeholder="Masukkan nama user..." class="w-full rounded-lg border @error('nama_user') border-red-500 focus:ring-red-500 @else border-gray-300 focus:border-brand-500 focus:ring-brand-500 @enderror bg-transparent px-4 py-3 text-sm text-gray-800 focus:outline-none focus:ring-1 dark:border-gray-700 dark:text-white/90 shadow-sm" required />
                    @error('nama_user')
                        <span class="text-xs text-red-500 mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select id="departemen" name="departemen" class="w-full rounded-lg border @error('departemen') border-red-500 focus:ring-red-500 @else border-gray-300 focus:border-brand-500 focus:ring-brand-500 @enderror bg-transparent px-4 py-3 text-sm text-gray-800 focus:outline-none focus:ring-1 dark:border-gray-700 dark:text-white/90 shadow-sm" required>
                        <option value="" disabled {{ old('departemen') ? '' : 'selected' }}>Pilih Departemen...</option>
                        @foreach ($departments as $department)
                            <option value="{{ $department->id }}" {{ old('departemen') == $department->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-brand-50/30 dark:bg-brand-500/5 p-5 rounded-xl border border-brand-100 dark:border-brand-500/20">
                ...
                <div>
                    ...
                    <input type="text" id="nomor_asset_pc" name="nomor_asset_pc" value="{{ old('nomor_asset_pc') }}" placeholder="Contoh: 51021000-0" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-3 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-1 dark:border-gray-700 dark:text-white/90 shadow-sm" />
                </div>

                <!-- Computer Name -->
                <div>
                    <label for="computer_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Computer Name</label>
                    <input type="text" id="computer_name" name="computer_name" value="{{ old('computer_name') }}" placeholder="Contoh: C022ACTUSER" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-3 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-1 dark:border-gray-700 dark:text-white/90 shadow-sm" />
                </div>

                <!-- Notes -->
                <div class="md:col-span-2">
                    <label for="notes" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Notes</label>
                    <textarea id="notes" name="notes" rows="3" placeholder="Tambahkan catatan jika diperlukan..." class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-3 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-1 dark:border-gray-700 dark:text-white/90 shadow-sm">{{ old('notes') }}</textarea>
                </div>
